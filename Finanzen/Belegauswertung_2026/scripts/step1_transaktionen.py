#!/usr/bin/env python3
"""
Schritt 1 - Transaktionen 2026 aus dem Finom-Kontoauszug (Ersatz fuer die nicht
existierende Lexoffice-Banktransaktions-API, s. Rueckfrage an Nutzer 2025).
Kategorisiert jede Transaktion konservativ; bei Unklarheit -> UNKLAR + PRUEFFALL.

Angepasst von Belegauswertung_2025/scripts/step1_transaktionen.py am 2026-07-28.
ALL_TX zeigt bewusst weiterhin auf den 2025er-Ordner: dort liegt die einzige
gepflegte Kopie der jahresuebergreifenden Rohdaten (_finom_alle_jahre.json).
Vor jedem Lauf pruefen, ob eine aktuellere Finom-Kontoauszug-PDF vorliegt und ggf.
_finom_alle_jahre.json neu aus scripts/parse_finom.py erzeugen (siehe NOTIZEN.md).
"""
import json
import re

ALL_TX = "/Users/sesp01-user/vault/Prozessia-Brain/Finanzen/Belegauswertung_2025/out/_finom_alle_jahre.json"
OUT = "/Users/sesp01-user/vault/Prozessia-Brain/Finanzen/Belegauswertung_2026/out/01_transaktionen.json"
...
# Liste 1:1 aus 2025 uebernommen als Startpunkt - im Jahresverlauf 2026 ergaenzen,
# sobald neue wiederkehrende Partner auftauchen.
...
def main():
    ...
    tx2026 = [t for t in all_tx if not t.get('parse_error') and t['zahlungsdatum'].startswith('2026')]
