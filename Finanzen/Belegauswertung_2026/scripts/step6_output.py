...
BASE = "/Users/sesp01-user/vault/Prozessia-Brain/Finanzen/Belegauswertung_2026/out"
...
ws.append(("HINWEIS:", "Schritt 1 (Lexoffice) hat keine Bank-Transaktions-API - Transaktionsgrundlage ist der vom Nutzer bereitgestellte Finom-Kontoauszug (Finom_statement_21072026.pdf, 29.10.2024-21.07.2026), auf 2026 gefiltert."))
ws.append(("HINWEIS:", "USt-Voranmeldungen 2026 werden vom Nutzer selbst geprueft, kein Abgleich in diesem Lauf."))
...
# ---------- Tab: Umsaetze_2026 (chronologisch, mit Beleg-Zuordnungsversuch) ----------
ws = wb.create_sheet("Umsaetze_2026")
...
rows = [
    ...
    ("Finom-Kontoauszug (Ersatz-Rueckgrat)", n_lex_tx, "Transaktionen 2026, gefiltert aus Gesamtauszug 29.10.2024-21.07.2026"),
    ("Lokaler Ordner (Belege)", n_lokal, f"davon {len(ausserhalb)} ausserhalb 2026 verschoben (out/ausserhalb_zeitraum.json)"),
    ("Google Drive (Finanzordner)", "~60+ Dateien gesichtet", f"Stichprobenartig ueber ~20 Ordner geprueft, {n_drive_confirmed_dupes}+ bestaetigte Duplikate lokaler Belege, {n_drive_only} Drive-only Fund (Finom 2024.pdf, ausserhalb 2026). Kein vollstaendiger rekursiver Scan aller ~50 Unterordner (siehe out/02b_belege_drive.json)."),
    ...
    ("Belege insgesamt (nach Dedup, in 2026)", len(belege_all), ""),
]
for r in rows:
    ws.append(list(r))
style_header(ws, len(headers))
...
wb.save(f"{BASE}/ergebnis_2026.xlsx")
print("Excel gespeichert:", f"{BASE}/ergebnis_2026.xlsx")
# Bewusst KEINE Kopie in den Jahresordner (Nutzerentscheidung 2026-07-22): dort
# soll nur noch ergebnis_2026_monatlich_netto.xlsx liegen (siehe step7), diese
# 10-Tab-Datei wird im Alltag nicht mehr genutzt und bleibt nur im out/-Ordner.

# ---------- PRUEFFAELLE.md ----------
pruef_rows.sort(key=lambda r: -(r[2] or 0))
lines = ["# Pruefaelle Belegauswertung Prozessia GbR 2026", "",
...
print(f"Transaktionen 2026 gesamt: {len(tx_all)}")
